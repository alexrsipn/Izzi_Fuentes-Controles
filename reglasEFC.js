...
class ValidationManager {
    ...
    async _fetchFromApi(endpoint) {
        if (!this.credentials) {
            throw new Error('Credenciales no establecidas. Use setCredentials antes de hacer peticiones.');
        }

        // Concatenar credenciales en formato adecuado
        var urlInstance = this.credentials.urlOFSC.replace('https://', '').split('.fs.ocs.oraclecloud.com')[0];
        var credentialsStr = this.credentials.ofscRestClientId + '@' + urlInstance + ':' + this.credentials.ofscRestSecretId;
        ...
        try {
            const response = await fetch(`${urlOFSC}/rest/ofscMetadata/v1/${endpoint}`, {
                method: 'GET',
                headers: {
    ...
    async loadRulesConfig() {
        try {
            ...
            const response = await this._fetchFromApi('properties/XA_REGLAS_EQ_FUENTE_CONTROL');

            // Procesar la respuesta JSON con códigos de escape
            const rawConfig = JSON.parse(response.transformation.xslt.replace(/\\"/g, '"'));
            ...
            if (Array.isArray(rawConfig)) {
                ...
                console.error('El formato de la configuración no es válido:', rawConfig);
            }
        } catch (error) {
            console.error('Error al cargar la configuración de reglas desde Metadata API:', error);
        }
    }
    ...
    generateSapIdList() {
        try {
            ...
            // ];
            const allMaterialTypes = this.rulesConfig.flatMap(rule => [...(rule.fuentes || []), ...(rule.controles || [])]);
            ...
            // ];
            this.sapIdList = [
                ...new Set(allEquipmentTypes.filter(id => id !== '*' && id !== '0' && id !== 'NA')),
                ...new Set(allMaterialTypes.filter(id => id !== '*' && id !== '0' && id !== 'NA'))
            ];
    ...
    filterInventoryByType(inventoryList, typeSapIds, type) {
        return inventoryList.flatMap((item, index) => {
            ...
            let typeProperty = type === 'equipo' ? 'XI_EQUIPMENTTYPE' : 'XI_MATERIALTYPE';
    ...
    validateCombination(equipment, sources, controls, registra_error, valida_e_fyoc_solos, validatedItems) {
        // let reglasAplicables = this.rulesConfig.filter(rule => rule.skuequipo === equipment.XI_EQUIPMENTTYPE);
        const applicableRule = this.rulesConfig.find(rule => rule.skuequipo === equipment.XI_EQUIPMENTTYPE)
        ...
        let fuentesValidas = new Set(applicableRule.fuentes.filter(f => f !== '0' && f !== 'NA'));
        let controlesValidos = new Set(applicableRule.controles.filter(c => c !== '0' && c !== 'NA'));
        ...
        for (const source of availableSources) {
            for (const control of availableControls) {
                if (fuentesValidas.has(source.XI_MATERIALTYPE) && controlesValidos.has(control.XI_MATERIALTYPE)) {
                    this.markAsUsed(equipment);
        ...
        if (applicableRule.controles.includes('0') || applicableRule.controles.includes('NA')) {
            for (const source of availableSources) {
                if (fuentesValidas.has(source.XI_MATERIALTYPE)) {
                    this.markAsUsed(equipment);
                    this.markAsUsed(source, false, 'fuente');
                    validatedItems.push({ equipo: equipment, fuente: source, control: null, resultado: true });
                    return true;
                }
            }
        }
        ...
        if (applicableRule.fuentes.includes('0') || applicableRule.fuentes.includes('NA')) {
            for (const control of availableControls) {
                if (controlesValidos.has(control.XI_MATERIALTYPE)) {
                    this.markAsUsed(equipment);
                    this.markAsUsed(control, false, 'control');
                    validatedItems.push({ equipo: equipment, fuente: null, control: control, resultado: true });
                    return true;
                }
            }
        }

        const noNecesitaFuente = applicableRule.fuentes.includes('0') || applicableRule.fuentes.includes('NA');
        const noNecesitaControl = applicableRule.controles.includes('0') || applicableRule.controles.includes('NA');

        if (noNecesitaFuente || noNecesitaControl) {
            this.markAsUsed(equipment);
            validatedItems.push({ equipo: equipment, fuente: null, control: null, resultado: true });
            return true;
        }
        ...
        if (registra_error) {
            this.errors.push(`❌ Para el equipo con número de serie ${equipment.serialNumber} (SKU ${equipment.XI_EQUIPMENTTYPE}) no se detectó fuente y/o control`);
        }
    ...
    validateInventory(inventarioInstalado, inventarioCliente) {
        ...
        let equipments = this.filterInventoryByType(inventarioInstalado, allowedEquipment, 'equipo');
        let sources = this.filterInventoryByType(inventarioInstalado, allowedSources, 'fuente');
        let controls = this.filterInventoryByType(inventarioInstalado, allowedControls, 'control');

        for (const equipment of equipments) {
            if (!this.isUsed(equipment)) {
                const isValid = this.validateCombination(equipment, sources, controls, false, false, validatedItems);
                if (!isValid) {
                    // 🔹 Ajustado al nuevo formato (serie + SKU)
                    this.errors.push(`❌ Para el equipo con número de serie ${equipment.serialNumber} (SKU ${equipment.XI_EQUIPMENTTYPE}) le falta fuente y/o control`);
                }
            }
        }

        // Guardar fuentes y controles que no fueron usados
        let remainingSources = sources.filter(src => !this.isUsed(src) && !this.usedWildcardSources.has(src.uniqueId));
        let remainingControls = controls.filter(ctrl => !this.isUsed(ctrl) && !this.usedWildcardControls.has(ctrl.uniqueId));
        console.log('Fuentes sobrantes después de validar con inventarios instalados:', remainingSources);
        console.log('Controles sobrantes después de validar con inventarios instalados:', remainingControls);

        // PARTE 2: Validar usando inventarios de cliente
        equipments = this.filterInventoryByType(inventarioCliente, allowedEquipment, 'equipo');
        sources = this.filterInventoryByType(inventarioInstalado, allowedSources, 'fuente');
        controls = this.filterInventoryByType(inventarioInstalado, allowedControls, 'control');

        for (const equipment of equipments) {
            if (!this.isUsed(equipment)) {
                this.validateCombination(equipment, sources, controls, false, true, validatedItems);
            }
        }

        // Identificar fuentes y controles sobrantes después de validar con inventarios de cliente
        remainingSources = sources.filter(src => !this.isUsed(src) && !this.usedWildcardSources.has(src.uniqueId));
        remainingControls = controls.filter(ctrl => !this.isUsed(ctrl) && !this.usedWildcardControls.has(ctrl.uniqueId));
        ...
        remainingSources.forEach(item => {
            this.errors.push(`❌ La fuente de poder con SKU ${item.XI_MATERIALTYPE} no acompaña a algún equipo`);
        });

        remainingControls.forEach(item => {
            this.errors.push(`❌ El control remoto con SKU ${item.XI_MATERIALTYPE} no acompaña a algún equipo`);
        });
    ...
    async fetchInstalledInventories(activityId) {
        ...
        try {
            const response = await fetch(`${urlOFSC}/rest/ofscCore/v1/activities/${activityId}/installedInventories`, {
                method: 'GET',
                headers: {
                    'Authorization': `Basic ${encodedCredentials}`,
                    'Accept': 'application/json'
            ...
            if (!response.ok) {
                throw new Error(`Error al obtener inventarios instalados: ${response.status} - ${response.statusText}`);
            }
            ...
            console.error('Error al obtener los inventarios instalados desde la API:', error);
            throw error;
        }
    }
    ...
    async fetchCustomerInventories(activityId) {
        ...
        try {
            const response = await fetch(`${urlOFSC}/rest/ofscCore/v1/activities/${activityId}/customerInventories`, {
                method: 'GET',
                headers: {
                    'Authorization': `Basic ${encodedCredentials}`,
                    'Accept': 'application/json'
            ...
            if (!response.ok) {
                throw new Error(`Error al obtener los inventarios de cliente: ${response.status} - ${response.statusText}`);
            }
            ...
            console.error('Error al obtener los inventarios de cliente desde la API:', error);
            return [];
        }
    }
    ...
    async consultarDescripcion(equipmentType) {
        ...
        try {
            ...
            var urlInstance = this.credentials.urlOFSC.replace('https://', '').split('.fs.ocs.oraclecloud.com')[0];
            const ofscHelper = new OFSCRequestHelper(urlInstance, 'Operación exitosa', 'Error en la operación');
            ofscHelper.setCredentials(this.credentials.ofscRestClientId, this.credentials.ofscRestSecretId);

            // Consultar la API de OFSC
            const response = await ofscHelper.request('GET', `ofscMetadata/v1/properties/XI_EQUIPMENTTYPE/enumerationList`);
            ...
            if (response.items && Array.isArray(response.items)) {
                response.items.forEach(item => {
                    if (item.label && item.translations && item.translations.length > 0) {
                        ...
                        this.equipmentDescriptionsCache[item.label] = item.translations.find(t => t.language === 'es')?.name ||
...
class OFSCRequestHelper {
    constructor(instance = '', successMessage = 'Operación exitosa', failureMessage = 'Error en la operación') {
        if (!instance) {
    ...
    setCredentials(clientId, secretId) {
        if (!clientId || !secretId) {
            throw new Error('clientId y secretId son requeridos para configurar credenciales.');
        }
        let credentialsStr = `${clientId}@${this.instance}:${secretId}`;
    ...
    async request(method, endpoint, body = null) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`${this.failureMessage}: ${response.status} - ${response.statusText}`);
            }
...
class ContoladorReglasEFC {
    ...
    async ejecutarValidacionEFC(receivedData) {
        const { valid, errors } = await this.validacEquiposFuenteControles(receivedData);
                    ...
                    <table border="1">
                        ...
                        <tbody>
                            ${valid.map((item, index) => `
                                <tr>
                                    <td>${index + 1}</td>
                                    <td>${item.equipo ? 'Equipo' : 'Fuente/Control'}</td>
                                    <td>${item.equipo?.XI_EQUIPMENTTYPE || item.fuente?.XI_MATERIALTYPE || item.control?.XI_MATERIALTYPE || 'Desconocido'}</td>
                                    <td>${item.descripcion || 'N/A'}</td>
                                    <td>${item.fuente ? item.fuente.XI_MATERIALTYPE : '-'}</td>
                                    <td>${item.control ? item.control.XI_MATERIALTYPE : '-'}</td>
                                    <td style="color:${item.resultado ? 'green' : 'red'};">${item.resultado ? '✅ Válido' : '❌ Inválido'}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
    ...
    async validacEquiposFuenteControles(receivedData) {
        // console.log('Iniciando validación de Equipos, Fuentes y Controles desde Mac');
        ...
        this.installedInventories = await this.validationManager.fetchInstalledInventories(this.plugin.aid);
        this.customerInventories = await this.validationManager.fetchCustomerInventories(this.plugin.aid);

        return this.validateAndFilterInventory(this.installedInventories, this.customerInventories);
    }

    validateAndFilterInventory(installedInventories, customerInventories) {
        // console.log('Validando inventarios instalados...');
        const validationResults = this.validationManager.validateInventory(installedInventories, customerInventories);
        this.installedInventories = validationResults.validatedItems;
        const errores = validationResults.errors;

        console.log('Colección de inventarios instalados válidos:', this.installedInventories);
    ...
    async populateInventoryTable(inventoryData, errors) {
        ...
        for (let index = 0; index < inventoryData.length; index++) {
            ...
            if (item.equipo) {
                identificador = item.equipo.XI_EQUIPMENTTYPE + " - " + item.equipo.XI_EQUIPMAKE || 'Desconocido';
                descripcion = await this.validationManager.consultarDescripcion(item.equipo.XI_EQUIPMENTTYPE);
            } else if (item.fuente || item.control) {
                identificador = item.fuente ? item.fuente.XI_MATERIALTYPE : item.control.XI_MATERIALTYPE;
                descripcion = await this.validationManager.consultarDescripcion(identificador);
            }

            let fuentesAsociadas = item.fuente ? (Array.isArray(item.fuente) ? item.fuente.map(f => f.XI_MATERIALTYPE).join(', ') : item.fuente.XI_MATERIALTYPE) : '-';
            let controlesAsociados = item.control ? (Array.isArray(item.control) ? item.control.map(c => c.XI_MATERIALTYPE).join(', ') : item.control.XI_MATERIALTYPE) : '-';
        ...
        if (errors.length > 0) {
            ...
            const evaluados = inventoryData.some(item => item.fuente || item.control);
            ...
            if(!evaluados){
                ...
                msgRow = msgRow + `<tr><td colspan="7" style="text-align: center; font-weight: bold;">✅ No se encontraron errores.</td></tr>`;
            }else{
