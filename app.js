(function ($) {
    ...
    $.extend(window.OfscPlugin.prototype, {
        ...
        pluginOpen: async function (receivedData) {
            ...
            if (userLogin) {
                this.userType = await this.obtenerTipoUsuario(userLogin, credentialsValidador);
            ...
            $('#loadConfigBtn').on('click', async function () {
                ...
                rulesConfig.forEach(rule => {
                    const fuentesStr = Array.isArray(rule.fuentes) ? rule.fuentes.join(", ") : (rule.fuentes || '');
                    const controlesStr = Array.isArray(rule.controles) ? rule.controles.join(", ") : (rule.controles || '');
                    const row = `
            <tr>
            ...
            if (!resvalefc) { return -1; }    //INCORPORACIÓN MAS LIMPIA DE REGLAS EFC ***SI SE NECESITA EN APROVISIONAMIENTO
        ...
        bindValidationButton: function () {     //***NO SE USARÁ EN PLUGIN DE APROVIIONAMIENTO */
            $('#validateBtn').off('click').on('click', () => {
                $('#statusMessage').text('Validando inventario...');

                const { valid, errors } = this.validateAndFilterInventory(this.installedInventories, this.customerInventories);
        ...
        obtenerTipoUsuario: async function (ulogin, credentialsValidador) {
            ...
            var urlInstance = credentialsValidador.urlOFSC.replace('https://', '').split('.fs.ocs.oraclecloud.com')[0];
            var credentialsStr = credentialsValidador.ofscRestClientId + '@' + urlInstance + ':' + credentialsValidador.ofscRestSecretId;
            ...
            try {
                const response = await fetch(`${urlOFSC}/rest/ofscCore/v1/users/${ulogin}`, {
                    method: 'GET',
                    headers: {
                ...
                return userData.userType && userData.userType.trim() !== '' ? userData.userType : 'No disponible';
        ...
        controlarVisibilidadTabs: function (tipoUsuario) {
            ...
            $('.tab').each(function () {
                ...
                if (tabPermisos[tipoUsuario] && tabPermisos[tipoUsuario].includes(tabId)) {
                    $(this).show();
                    $('#' + tabId).show();
                } else {
                    $(this).hide();
        ...
        handleFileUpload: function () {
            ...
            if (!file) {
                $('#uploadStatusMessage').text('❌ No se seleccionó ningún archivo.').css('color', 'red');
                return;
            }
            ...
            reader.onload = (event) => {
                try {
                    ...
                    const transformedData = jsonData.map(row => {
                        const fuentes = (row.skuequipo || '').toString().split(',').map(f => f.trim()).filter(Boolean);
                        const controles = (row.skucontrol || '').toString().split(',').map(c => c.trim()).filter(Boolean);
                    ...
                    console.log('Datos cargados desde el archivo:', self.validationManager.rulesConfig);
                    $('#uploadStatusMessage').text('✅ Archivo cargado y procesado correctamente.').css('color', 'green');
                    $('#loadConfigBtn').click(); // Simular clic para cargar la configuración y actualizar la tabla
                } catch (error) {
                    console.error('Error al leer el archivo:', error);
                    $('#uploadStatusMessage').text('❌ Error al leer el archivo.').css('color', 'red');
                }
            };
